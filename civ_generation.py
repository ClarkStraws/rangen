...
def generate_tribes(life_forms: list[LifeForm], map_data: dict) -> list[Tribe]:
    ...
    for life in life_forms:
        ...
        for _ in range(num_tribes):
            tribe_name = f"{life.name} Tribe-{random.randint(1, 100)}"
            ...
            tribes.append(tribe)
            print(f"Generated tribe: {tribe.name} on {tribe.home_planet.name} at location {tribe.location} with traits {tribe.location_trait}, {tribe.resource_trait}, religion scale {tribe.religion_scale:.2f}, social scale {tribe.social_scale:.2f}")
